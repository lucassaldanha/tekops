...
Every `beacon` subcommand, plus `peers`, accepts `--json` to print a
JSON-serialized version of the parsed response instead of the summary/table
(not a raw passthrough of the Beacon API's wire response - e.g. `peers --json`
includes a `protocol` field that's derived locally, not sent by the API), and
`--api-url` (or `$TEKOPS_API_URL`) to point at a non-default Beacon API
