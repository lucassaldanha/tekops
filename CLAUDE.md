...
- **`cli.rs`** — all `clap` command/subcommand definitions and dispatch. Every `beacon` subcommand (and the top-level `peers`/`health` commands) follows the same shape: call one or more `BeaconClient` methods, then either print a `serde_json`-serialized struct (`--json`) or hand the result to an `output::format_*` function. `exit_for` centralizes error handling — each `beacon_*`/`peers`/`health` handler returns `Result<(), ApiError>` and `exit_for` prints `error: {e}` and sets the exit code. `peers` and `health` live at the top level (`tekops peers`, `tekops health` — not nested under `beacon`) since they're used far more often than the other `beacon` subcommands.
